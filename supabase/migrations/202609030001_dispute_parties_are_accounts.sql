-- Dispute parties moved to the OpenLC identity model: buyer_id and supplier_id are
-- openlc_accounts ids, and arbitrator_id is a configured id that is not an account at all.
...
comment on column public.dispute_aggregates.buyer_id is 'OpenLC account id, not an auth.users id.';
comment on column public.dispute_aggregates.supplier_id is 'OpenLC account id, not an auth.users id.';
